...
Public Class frmStaffLoanCollection
    ...
    Private Sub frmStaffLoanCollection_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        FormShown(Me)
    End Sub

    Private Sub frmStaffLoanCollection_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cmbMonth.SelectedIndex = 0
        cmbDemandType.SelectedIndex = 0
        cmbTokenNo.RecordSet(objDT("sp_Loanmember_getall @staff=1"), "strEmpno", "MemberCode")
    ...
    Private Sub btnView_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnView.Click
        ...
        If cmbTokenNo.SelectedIndex = 0 Then
            Try
                ...
                If cmbTokenNo.SelectedIndex = 0 Then
                    objColl = objDT("sp_Collection @StaffLoan=1 ,@DemandType=" & cmbDemandType.SelectedIndex & ",@Fromdate='" & SD(dtpFrom.Value) & "',@Todate='" & SD(dtpTo.Value) & "'")
                Else
                    objColl = objDT("sp_Collection @StaffLoan=1 ,@MemberCode=" & cmbTokenNo.ColData("MemberCode") & ",@DemandType=" & cmbDemandType.SelectedIndex & ",@Fromdate='" & SD(dtpFrom.Value) & "',@Todate='" & SD(dtpTo.Value) & "'")
                End If
        ...
        Else
            Try
                Dim objColl As DataTable
                objColl = objDT("sp_Collection @MemberCode=" & cmbTokenNo.ColData("MemberCode") & ",@DemandType=" & cmbDemandType.SelectedIndex & ",@MaxDemandCode=1,@MonthNo=" & Month(dtpTo.Value) & ",@YearNo=" & Year(dtpTo.Value))
                If objColl.Rows.Count = 0 Then
                    objColl = objDT("sp_Collection @MemberCode=" & cmbTokenNo.ColData("MemberCode") & ",@DemandType=" & cmbDemandType.SelectedIndex & ",@Fromdate='" & SD(dtpFrom.Value) & "',@Todate='" & SD(dtpTo.Value) & "',@MonthNo=" & Month(dtpTo.Value) & ",@YearNo=" & Year(dtpTo.Value))
                End If
                If objColl.Rows.Count = 0 Then
                    objColl = objDT("sp_Collection @MemberCode=" & cmbTokenNo.ColData("MemberCode") & ",@DemandType=" & cmbDemandType.SelectedIndex & ",@Fromdate='" & SD(dtpFrom.Value) & "',@Todate='" & SD(dtpTo.Value) & "',@MonthNo=" & Month(dtpTo.Value) & ",@YearNo=" & Year(dtpTo.Value))
                End If
    ...
    Private Sub CalsiTotal()
        If cmbTokenNo.SelectedIndex = 0 Then
            ...
            If Val(txtDemandAmt.Text) = 0 And SD(dtpTo.Value) = SD(dtpUpdationDate.Value) Then
    ...
    Private Sub Grid1_CellMouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles Grid1.CellMouseClick
        ...
        If e.ColumnIndex = Grid1.Columns("Demand").Index Then
            ...
            Dim opnDemand As DataTable = objDT("Select * from tbl_LoanDemand where  staffLoan=1 and DateFrom >='" & SD(dtpTo.Value) & "' ") 'having MAX(yearno)>0 and max(MonthNo)>0") 'NewDemand=0 and 
            ...
            Dim objLoan As DataTable = objDT("sp_MemberLoan_GetAll  @MemberTYpe='STAFF',@LoanCode=" & Val(Grid1.CurrentRow.Cells("LoanCode").Value))
            If objLoan.Rows.Count > 0 Then
                For Rowid As Integer = 0 To objLoan.Rows.Count - 1
                    ...
                    If SD(objLoan.Rows.Item(Rowid)("MaturityDate")) > SD(DFrom) Or objLoan.Rows.Item(Rowid)("LoanClossedStatus") = False Then
                        ...
                        Dim SQuery As String = "sp_LoanDemand_GetAll @StaffLoan=1,@LoanCode=" & objLoan.Rows.Item(Rowid)("LoanCode")  '--& ", @DateTo='" & SD(DPTo) & "',@DateFrom='" & SD(DPFrom) & "'"
                        ...
                        If objdemand.Rows.Count > 0 Then
                            ...
                            DGrid.Item("StaffLoan", DGrid.Rows.Count - 1).Value = 1
                            DGrid.Item("WorkerLoan", DGrid.Rows.Count - 1).Value = 0

                        Else

                            SQuery = "sp_MemberLoan_GetAll @MemberType='STAFF',@LoanCode=" & objLoan.Rows.Item(Rowid)("LoanCode") & ",@MemberCode=" & objLoan.Rows.Item(Rowid)("MemberCode") & ",@FromDate='" & SD(dtpFrom.Value) & "',@ToDate='" & SD(dtpTo.Value) & "'"
                            objdemand = objDT(SQuery)
        ...
        If CurrInt > 0 Then
            ...
            Try
                For Rowindex As Integer = 0 To DGrid.Rows.Count - 1
                    ...
                    objcommand.Parameters.AddWithValue("@WorkerLoan ", 0)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If Val(txtCollectionAmt.Text) <> Val(txtMillColection.Text) Then
            ...
            End If
        End If
        If pnlUpDate.Enabled = True Then
            If dtpUpdationDate.Value > dtpCollectionDate.Value Then
                MsgBox("Check CollectionDate Please ( Updation Date is More than CollectionDate)...", MsgBoxStyle.Information, Mssg)
                dtpCollectionDate.Focus()
                Exit Sub
            End If
        End If
                        ...
                        End If
                            ...
                            For RowID As Integer = 0 To objLoan.Rows.Count - 1
                                ...
                                If Amt > 0 Then 'Val(objLoan.Rows.Item(RowID)("Principal"))
                                    ...
                                    ''''''''''''''''''objCommand1.Parameters.AddWithValue("@NewLine", 1)
                                    objCommand1.Parameters.AddWithValue("@CollectionDate", SD(CollDate))
                                    objCommand1.Parameters.AddWithValue("@ODPrincipal", ODPrn)
                                    objCommand1.Parameters.AddWithValue("@ODInterest", ODInt)
                                    objCommand1.Parameters.AddWithValue("@Principal", CollPrn)
    ...
    Private Sub dtpUpdationDate_IsOverwriteModeChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles dtpUpdationDate.IsOverwriteModeChanged
