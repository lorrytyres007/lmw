...
Public Class frmShareClossing
    Dim sTime As Integer = 0
    Dim Bool_Edit As Boolean
    Private Sub frmShareClossingvb_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If Bool_Edit = False Then
            optShareClossing.Enabled = True
            optRefund.Enabled = True
            optShareClossing.Checked = True
        End If
    End Sub
    ...
    Public Sub Show_Edit(ByVal DataRow As DataGridViewRow)
        Try
            Return
            Bool_Edit = True
            optShareClossing.Enabled = False
            optRefund.Enabled = False
            If optShareClossing.Checked = True Then
                dtpDepositClossingDate.Value = DataRow.Cells("ShareClossingDate").Value
                ...
                txtAmount.Text = DataRow.Cells("ClossingAmount").Value
            ...
            If optRefund.Checked = True Then
                ...
                txtOpening.Text = DataRow.Cells("OpeningAmt").Value
                txtCollection.Text = DataRow.Cells("TDCollection").Value
                txtAmount.Text = DataRow.Cells("RefundAmount").Value
            End If
            Me.ShowDialog()
    ...
    Private Sub txtEmpNo_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtEmpNo.Validated
        If Val(txtEmpNo.Text) > 0 Then
            ...
            txtOpening.Text = sEmpNo.Rows.Item(0)("ShareOpening")
            txtCollection.Text = objDScal("select isnull(sum(AMount),0) from tbl_ShareDeposit where membercode=" & Val(txtMemberCode.Text))
            txtTotal.Text = Val(txtOpening.Text) + Val(txtCollection.Text)
            txtAmount.Text = Val(txtTotal.Text)

            'Grid.RecordSet(objDT("sp_ShareCollection @Roi=" & Val(txtROI.Text) & ",@Fromdate='"& dtp',@TOdate='2015-08-20',@Membercode=373
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        End If
        If optShareClossing.Checked = True Then
            Dim objCommand As New SqlCommand("sp_ShareClossing_AddEdit", objSqlConnection)
            objCommand.CommandType = CommandType.StoredProcedure


            If Bool_Edit = True Then
                objCommand.Parameters.AddWithValue("@ShareClossingCode", Val(txtShareClossingCode.Text))
            End If

            objCommand.Parameters.AddWithValue("@ShareClossingDate", SD(dtpDepositClossingDate.Value))
            ...
            frmShareClossingDetails.Grid.RecordSet(objDT("sp_ShareClossing_GetAll"), "ShareClossingCode,MemberCode")
            MsgBox("Record Saved Successfully....", MsgBoxStyle.Information, Mssg)
            btnClear_Click(sender, e)
        End If
        If optRefund.Checked = True Then

            Dim objCommand As New SqlCommand("sp_ShareRefund_AddEdit", objSqlConnection)
            objCommand.CommandType = CommandType.StoredProcedure
            ...
            objCommand.Parameters.AddWithValue("@RefundAmount", Val(txtAmount.Text))
            objCommand.Parameters.AddWithValue("@User", int_UserCode)
            objCommand.Parameters.AddWithValue("@Node", int_NodeCode)
            objCommand.ExecuteNonQuery()
            frmShareClossingDetails.Grid1.RecordSet(objDT("sp_ShareRefund_GetAll"), "RefundCode,MemberCode")
            MsgBox("Record Saved Successfully....", MsgBoxStyle.Information, Mssg)
            btnClear_Click(sender, e)
    ...
    Private Sub optShareClossing_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles optShareClossing.CheckedChanged
        txtAmount.Enabled = False
    End Sub

    'Private Sub txtRefundAmt_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtAmount.Validated
    '    If Val(txtAmount.Text) > 0 Then
    '        Dim tot As Double = Val(txtOpening.Text) + Val(txtCollection.Text)
    '        If Val(txtAmount.Text) > tot Then
