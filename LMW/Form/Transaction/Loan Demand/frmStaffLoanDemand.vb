...
Public Class frmStaffLoanDemand
    ...
    Private Sub frmStaffLoanDemand_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        FormShown(Me)
    End Sub

    Private Sub frmStaffLoanDemand_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If objLoan.Rows.Count > 0 Then
            For Rowid As Integer = 0 To objLoan.Rows.Count - 1
                ...
                If SD(objLoan.Rows.Item(Rowid)("MaturityDate")) > SD(DFrom) Then
                    ...
                    Grid.Item("StaffLoan", Grid.Rows.Count - 1).Value = 1
                    Grid.Item("WorkerLoan", Grid.Rows.Count - 1).Value = 0
                    'End If
                End If
            Next
        End If
    ...
    Private Sub btnCalculation_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCalculation.Click
        If cmbMonth.SelectedIndex > 0 Then
            ...
            End If
            Dim opnDemand As DataTable = objDT("Select * from tbl_LoanDemand where StaffLoan=1 and Yearno=" & Year(dtpTo.Value) & "and Monthno=" & Month(dtpTo.Value)) ' & "  & " and  DateFrom >='" & SD(dtpFrom.Value) & "' ") 'having MAX(yearno)>0 and max(MonthNo)>0") 'NewDemand=0 and 
            ...
            If Opening = True Then
                objdemand = objDT("sp_LoanDemand_GetAll @StaffLoan=1 , @Yearno=" & Year(dtpTo.Value) & " , @Monthno=" & Month(dtpFrom.Value) & " ,@DateFrom ='" & SD(dtpFrom.Value) & "' ") 'having MAX(yearno)>0 and max(MonthNo)>0") 'NewDemand=0 and 
                If objdemand.Rows.Count > 0 Then
                ...
                Dim Days As Integer = 0
                Dim Mons As Integer = 0
                Dim objLoan As DataTable = objDT("sp_MemberLoan_GetAll  @MemberTYpe='STAFF'") ',@Membercode=1993")
                If objLoan.Rows.Count > 0 Then
                    For Rowid As Integer = 0 To objLoan.Rows.Count - 1
